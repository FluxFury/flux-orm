...
async def test_setup_sports():
    async with new_session() as session:
        ...
        sports = [
            ...
            Sport(
                name="Basketball",
                description="A sport with a ball and a basket",
                image_url="basketball.png",
            ),
            Sport(name="Test Sport", description="A test sport", image_url=None),
            Sport(
                name="Sport with Competitions",
                description="Sport for competitions",
                image_url=None,
            ),
            Sport(name="New Sport", description="Another new sport", image_url=None),
            Sport(
                name="Team Competition Sport",
                description="Sport for team competition",
                image_url=None,
            ),
            Sport(
                name="Competition for Match Sport",
                description="Sport for competition matches",
                image_url=None,
            ),
            Sport(
                name="Category Sport",
                description="Sport for category tests",
                image_url=None,
            ),
            Sport(
                name="AI Statement Sport",
                description="Sport for AI statement tests",
                image_url=None,
            ),
            Sport(
                name="Delete Test Sport",
                description="Sport for delete tests",
                image_url=None,
            ),
        ]
...
async def test_cascade_delete_competition():
    async with new_session() as session:
        ...
        match = Match(
            match_name="Test Match", competition=competition, match_status=status
        )
        ...
        result = await session.execute(
            select(Team).filter(Team.name.in_(["Team X", "Team Y"]))
        )
        teams = result.unique().scalars().all()
        print(teams)
        assert len(teams) == 2, (
            "Teams should not be deleted when competition is deleted"
        )
...
async def test_cascade_save_team_with_members():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(TeamMember).filter(TeamMember.name.in_(["Player One", "Player Two"]))
        )
...
async def test_delete_team_and_check_members():
    async with new_session() as session:
        ...
        assert remaining_member is None, (
            "TeamMember should be deleted when team is deleted"
        )
...
async def test_update_coach_and_check_teams():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(Team).filter(Team.name.in_(["Team H", "Team I"]))
        )
        teams = result.unique().scalars().all()
        for team in teams:
            assert updated_coach in team.coaches, (
                f"{team.name} should have the updated coach"
            )
...
async def test_add_match_with_teams():
    async with new_session() as session:
        ...
        match = Match(
            match_name="Team J vs Team K", competition=competition, match_status=status
        )
        match.match_teams = [team1, team2]
        session.add(match)
        await session.commit()
        ...
        result = await session.execute(
            select(Match).filter_by(match_name="Team J vs Team K")
        )
...
async def test_cascade_delete_team_member():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(TeamMember).filter_by(name="Transient Player")
        )
        ...
        result = await session.execute(
            select(Team).filter(Team.name.in_(["Team L", "Team M"]))
        )
        teams = result.unique().scalars().all()
        ...
        for team in teams:
            assert member not in team.members, (
                "Teams should not have the deleted member"
            )
...
async def test_create_match_with_status_and_substitutions():
    async with new_session() as session:
        ...
        # Create Match
        match = Match(
            match_name="Team O vs Team P", competition=competition, match_status=status
        )
        match.match_teams = [team1, team2]
        session.add(match)
        await session.flush()
        ...
        result = await session.execute(
            select(Substitution).filter_by(match_id=match.match_id)
        )
...
async def test_cascade_delete_match():
    async with new_session() as session:
        ...
        match = Match(
            match_name="Team Q vs Team R", competition=competition, match_status=status
        )
        match.match_teams = [team1, team2]
        session.add(match)
        await session.commit()
        ...
        result = await session.execute(
            select(Match).filter_by(match_name="Team Q vs Team R")
        )
        ...
        assert not substitutions, (
            "Substitutions should be deleted when match is deleted"
        )

        # Verify that MatchStatus still exists
        result = await session.execute(
            select(MatchStatus).filter_by(name="In Progress")
        )
        status_exists = result.scalars().first()
        assert status_exists is not None, (
            "MatchStatus should not be deleted when match is deleted"
        )
...
async def test_relationship_team_competition():
    async with new_session() as session:
        # Fetch existing sport
        result = await session.execute(
            select(Sport).filter_by(name="Team Competition Sport")
        )
        sport = result.scalars().first()
        ...
        session.add(competition)
        await session.commit()
        competition = await session.execute(
            select(Competition).filter_by(name="Team Competition Test")
        )
...
async def test_cascade_delete_coach():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(Coach).filter_by(name="Coach Delete Test")
        )
        ...
        # Verify that Teams still exist
        result = await session.execute(
            select(Team).filter(Team.name.in_(["Team V", "Team W"]))
        )
        teams = result.unique().scalars().all()
...
async def test_cascade_save_competition_with_categories():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(Competition).filter_by(name="Competition with Categories")
        )
        comp = result.scalars().first()
...
async def test_cascade_delete_sport_with_competitions():
    async with new_session() as session:
        # Fetch existing sport
        result = await session.execute(
            select(Sport).filter_by(name="Sport with Competitions")
        )
        sport = result.scalars().first()
        ...
        result = await session.execute(
            select(Competition).filter(
                Competition.name.in_(["Competition A", "Competition B"])
            )
        )
        competitions = result.scalars().all()
        assert len(competitions) == 0, (
            "Competitions should be deleted when sport is deleted"
        )

        # Verify that Competitions no longer have the sport associated
        for competition in competitions:
            assert competition.sport_id != sport.sport_id, (
                "Competitions should not reference the deleted sport"
            )
...
async def test_cascade_delete_competition_category():
    async with new_session() as session:
        ...
        competition = Competition(
            name="Competition for Category Deletion", sport=sport, categories=[category]
        )
        session.add(competition)
        await session.commit()
        ...
        result = await session.execute(
            select(Competition).filter_by(name="Competition for Category Deletion")
        )
        remaining_competition = result.scalars().first()
        assert remaining_competition is not None, (
            "Competition should not be deleted when category is deleted"
        )

        # Проверяем, что соревнование больше не связано с категорией
        assert len(remaining_competition.categories) == 0, (
            "Competition should no longer have the deleted category"
        )
...
async def test_cascade_delete_match_status():
    async with new_session() as session:
        ...
        # Получаем существующий спорт
        result = await session.execute(
            select(Sport).filter_by(name="Delete Test Sport")
        )
        sport = result.scalars().first()
        ...
        match = Match(
            match_name="Match with Deletable Status",
            competition=competition,
            match_status=status,
        )
        session.add(match)
        ...
        result = await session.execute(
            select(Match).filter_by(match_name="Match with Deletable Status")
        )
        remaining_match = result.scalars().first()
        assert remaining_match is not None, (
            "Match should not be deleted when status is deleted"
        )

        # Проверяем, что матч больше не имеет статуса
        assert remaining_match.match_status is None, (
            "Match should no longer have the deleted status"
        )
...
async def test_delete_substitution():
    async with new_session() as session:
        ...
        competition = Competition(
            name="Competition for Substitution Deletion", sport=sport
        )
        session.add(competition)
        await session.commit()
        ...
        session.add_all([player_in, player_out])
        await session.commit()
        player_in = await session.execute(
            select(TeamMember).filter_by(name="Sub In Player")
        )
        player_in = player_in.scalars().first()
        player_out = await session.execute(
            select(TeamMember).filter_by(name="Sub Out Player")
        )
        player_out = player_out.scalars().first()
        substitution = Substitution(
            match=match,
        ...
        match = await session.execute(
            select(Match).filter_by(match_name="Match for Substitution Deletion")
        )
        ...
        remaining_match = result.scalars().first()
        assert remaining_match is not None, (
            "Match should still exist after substitution is deleted"
        )

        result = await session.execute(
            select(Team).filter_by(name="Team for Substitution")
        )
        remaining_team = result.scalars().first()
        assert remaining_team is not None, (
            "Team should still exist after substitution is deleted"
        )
...
async def test_cascade_delete_ai_statement():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(Sport).filter_by(name="AI Statement Sport")
        )
        sport = result.scalars().first()
        assert sport is not None, "AI Statement Sport should exist"

        competition = Competition(
            name="Competition for AI Statement Deletion", sport=sport
        )
        session.add(competition)
        await session.commit()
        ...
        # Проверяем, что матч все еще существует
        result = await session.execute(
            select(Match).filter_by(match_name="Match with AI Statement")
        )
        remaining_match = result.scalars().first()
        assert remaining_match is not None, (
            "Match should still exist after AI statement is deleted"
        )

        # Проверяем, что матч больше не связан с AI заявлением
        assert len(remaining_match.ai_statements) == 0, (
            "Match should no longer have the deleted AI statement"
        )
...
async def test_update_team_member():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(TeamMember).filter_by(name="Updatable Player")
        )
        updated_member = result.scalars().first()
        assert updated_member.age == 26, "Player's age should be updated to 26"
        assert updated_member.country == "Country B", (
            "Player's country should be updated to Country B"
        )
...
async def test_add_remove_team_member_from_team():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(TeamMember).filter_by(name="Dynamic Player 1")
        )
        remaining_member = result.scalars().first()
        assert remaining_member is not None, (
            "Removed member should still exist in the database"
        )
...
async def test_add_remove_coach_from_team():
    async with new_session() as session:
        ...
        assert remaining_coach is not None, (
            "Removed coach should still exist in the database"
        )
...
async def test_cascade_delete_competition_with_matches():
    async with new_session() as session:
        ...
        match1 = Match(
            match_name="Match 1", competition=competition, match_status=status
        )
        match2 = Match(
            match_name="Match 2", competition=competition, match_status=status
        )
        ...
        result = await session.execute(
            select(Competition).filter_by(name="Competition with Matches")
        )
...
async def test_cascade_delete_sport_with_categories():
    async with new_session() as session:
        ...
        competition1 = Competition(
            name="Competition X", sport=sport, categories=[category1]
        )
        competition2 = Competition(
            name="Competition Y", sport=sport, categories=[category2]
        )
        session.add_all([competition1, competition2])
        await session.commit()
        ...
        result = await session.execute(
            select(Competition).filter(
                Competition.name.in_(["Competition X", "Competition Y"])
            )
        )
        competitions = result.scalars().all()
        ...
        result = await session.execute(
            select(CompetitionCategory).filter(
                CompetitionCategory.name.in_([
                    "Sport Deletion Category 1",
                    "Sport Deletion Category 2",
                ])
            )
        )
        categories = result.scalars().all()
        assert len(categories) == 0, (
            "CompetitionCategories should be deleted when sport is deleted"
        )
...
async def test_update_match_status():
    async with new_session() as session:
        ...
        result = await session.execute(
            select(Match).filter_by(match_name="Match for Status Update")
        )
        updated_match = result.scalars().first()
        assert updated_match.match_status.name == "Updated Status", (
            "MatchStatus should be updated in the match"
        )
...
async def test_delete_team_in_competition():
    async with new_session() as session:
        ...
        competition = await session.execute(
            select(Competition).filter_by(name="Competition for Team Deletion")
        )
        competition = competition.scalars().first()
        ...
        remaining_competition = result.scalars().first()
        assert remaining_competition is not None, (
            "Competition should still exist after team is deleted"
        )

        # Проверяем, что соревнование больше не содержит команду
        assert len(remaining_competition.teams) == 0, (
            "Competition should not have the deleted team"
        )
...
async def test_cascade_delete_player_in_team():
    async with new_session() as session:
        ...
        team = await session.execute(
            select(Team).filter_by(name="Team with Player Deletion")
        )
        team = team.scalars().first()
        ...
        result = await session.execute(
            select(TeamMember).filter_by(name="Player to Delete")
        )
        deleted_member = result.scalars().first()
        ...
        remaining_team = result.scalars().first()
        assert remaining_team is not None, (
            "Team should still exist after player is deleted"
        )

        # Проверяем, что команда больше не содержит игрока
        assert len(remaining_team.members) == 0, (
            "Team should not have the deleted player"
        )
...
async def test_cascade_delete_coach_in_team():
    async with new_session() as session:
        ...
        team = await session.execute(
            select(Team).filter_by(name="Team with Coach Deletion")
        )
        team = team.scalars().first()
        ...
        remaining_team = result.scalars().first()
        assert remaining_team is not None, (
            "Team should still exist after coach is deleted"
        )

        # Проверяем, что команда больше не содержит тренера
        assert len(remaining_team.coaches) == 0, (
            "Team should not have the deleted coach"
        )
...
async def test_cascade_delete_match_with_ai_statements():
    async with new_session() as session:
        ...
        sport = result.scalars().first()
        competition = Competition(
            name="Competition for Match Deletion with AI", sport=sport
        )
        session.add(competition)
        await session.commit()
        ...
        match = Match(
            match_name="Match to Delete with AI",
            competition=competition,
            match_status=status,
        )
        session.add(match)
        ...
        result = await session.execute(
            select(Match).filter_by(match_name="Match to Delete with AI")
        )
        deleted_match = result.scalars().first()
        assert deleted_match is None, "Match should be deleted"
        ...
        for ai_stmt in ai_statements:
            assert match not in ai_stmt.matches, (
                "AI Statement should not reference the deleted match"
            )
...
async def test_cascade_delete_team_with_substitutions():
    async with new_session() as session:
        ...
        sport = result.scalars().first()
        competition = Competition(
            name="Competition for Team Deletion with Substitutions", sport=sport
        )
        session.add(competition)
        await session.commit()
        ...
        await session.commit()
        player_in = await session.execute(
            select(TeamMember).filter_by(name="Sub Player In")
        )
        player_in = player_in.scalars().first()
        player_out = await session.execute(
            select(TeamMember).filter_by(name="Sub Player Out")
        )
        player_out = player_out.scalars().first()
        substitution = Substitution(
            match=match,
        ...
        # Проверяем, что команда удалена
        result = await session.execute(
            select(Team).filter_by(name="Team with Substitutions")
        )
        deleted_team = result.scalars().first()
        assert deleted_team is None, "Team should be deleted"
        match = await session.execute(
            select(Match).filter_by(match_name="Match with Substitutions")
        )
        match = match.scalars().first()
        ...
        remaining_match = result.scalars().first()
        assert remaining_match is not None, (
            "Match should still exist after team is deleted"
        )
